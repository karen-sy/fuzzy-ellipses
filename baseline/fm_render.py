...
def render_func_quat_cam(
    means, prec_full, weights_log, pixel_list, aspect, invF, quat, t, beta_2, beta_3
):
    camera_rays = (pixel_list - jnp.array([0.5, 0.5, 0])) * jnp.array(
        [invF, aspect * invF, 1]
    )
...
def render_func_idx_quattrans(
    means, prec_full, weights_log, pixel_posei, invF, poses, beta_2, beta_3
):
    ...
    return render_func_rays(
        means, prec_full, weights_log, camera_rays_start, beta_2, beta_3
    )


# renders a batch of rays, as above, but also computes fwd & backward flow for each pixel.
def render_func_idx_quattrans_flow(
    means, prec_full, weights_log, pixel_posei, invF, poses, beta_2, beta_3
):
    rot_mats = jax.vmap(quat_to_rot)(poses[:, :4])
    ...
    # render the pixels
    est_depth, est_alpha, est_norm, est_w = render_func_rays(
        means, prec_full, weights_log, camera_rays_start, beta_2, beta_3
    )
