...
__device__ float nan_to_num(float x){
    if (isnan(x)){
        return 0.0f;
    } if (isinf(x)){
...
__device__ void sum_vec(float* a, float* c, const int N, const float init=0.0f){
    // store sum of a in c
...
__device__ void sub_scalar(float* a, float b, float* c, const int N){
    auto ff = [b] __device__ (float x) {return x - b;};
    thrust::transform(thrust::device, a, a + N, c, ff);
}
...
__device__ void positive_mask_vec(float* a, float* c, const int N){
    auto ff = [] __device__ (float x) {return x > 0 ? 1.0f : 0.0f;};
    thrust::transform(thrust::device, a, a + N, c, ff);
}

__device__ void clipped_exp(float* a, float* c, const int N){
