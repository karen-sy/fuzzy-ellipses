...
void render_func_rays(
    ...
    // T0DO it would be nice to reduce this number by culling gaussians with < threshold pdf value
    ...
    dim3 N_BLOCKS_PER_GRID(width/TILE_WIDTH, height/TILE_HEIGHT);

    printf("Launching kernel with (%d,%d,%d)=%d blocks per grid\n", N_BLOCKS_PER_GRID.x, N_BLOCKS_PER_GRID.y, N_BLOCKS_PER_GRID.x*N_BLOCKS_PER_GRID.y);
...
void render_func_quat(
    ...
    // intermediate values to compute in-kernel.
